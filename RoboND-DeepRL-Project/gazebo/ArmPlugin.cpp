...
// number of Actions
#define NUM_ACTIONS 6 // 3 joints, and two actions per joint: increase and decrease (if action is odd => decrease, if action is even => increase)
...
#define COLLISION_POINT1  "arm::gripperbase::gripper_link"
#define COLLISION_POINT2  "arm::gripper_middle::middle_collision"
#define COLLISION_POINT3  "arm::gripper_right::right_gripper"
#define COLLISION_POINT4  "arm::gripper_left::left_gripper"
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		/*
		/ TODO - Check if there is collision between the gripper and object, then issue learning reward
		/
		*/
		
		bool gripperCollisionCheck;
		if ((strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT1) == 0) || (strcmp(contacts->contact(i).collision1().c_str(), COLLISION_POINT1) == 0))
			gripperCollisionCheck = true;
		else if ((strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT2) == 0) || (strcmp(contacts->contact(i).collision1().c_str(), COLLISION_POINT2) == 0))
			gripperCollisionCheck = true;
		else if ((strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT3) == 0) || (strcmp(contacts->contact(i).collision1().c_str(), COLLISION_POINT3) == 0))
			gripperCollisionCheck = true;
		else if ((strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT4) == 0) || (strcmp(contacts->contact(i).collision1().c_str(), COLLISION_POINT4) == 0))
			gripperCollisionCheck = true;
		else
			gripperCollisionCheck = false;
		if (gripperCollisionCheck)
		{
			...
			newReward = true;
			endEpisode = true;
			printf("gripper_prop Collision: rewardHistory = %f\n", rewardHistory);
			return;
		}
		...
		*/
		...
		if (collisionCheck)
		{
			...
			newReward  = true;
			endEpisode = true;
			printf("item Collision: rewardHistory = %f\n", rewardHistory);
			return;
		}
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( maxEpisodeLength > 0 && episodeFrames > maxEpisodeLength )
	{
		...
		newReward     = true;
		endEpisode    = true;
		printf("episode timeout: rewardHistory = %f\n", rewardHistory);
	}
	...
	if( hadNewState && !newReward )
	{
		...
		/
		*/
		bool checkGroundContact = (gripBBox.min.z > groundContact) ? false : true;
		...
		if(checkGroundContact)
		{
			...
			newReward     = true;
			endEpisode    = true;
			printf("hitting ground: rewardHistory = %f\n", rewardHistory);
		}
		...
		*/ 
		...
		if(!checkGroundContact)
		{
			...
			if( episodeFrames > 1 ) // if it is not the first frame, lastGoalDistance has a correct value
			{
				const float distDelta  = lastGoalDistance - distGoal; // if arm get farther from goal, distDelta => -ive
				
				printf("distDelta = %f\n", distDelta);
				// compute the smoothed moving average of the delta of the distance to the goal
				float alpha = 0.2;
				avgGoalDelta  = (avgGoalDelta * alpha) + (distDelta * (1 - alpha)); // decrease if we get further from goal
				rewardHistory = 25*avgGoalDelta;
				if (abs(distDelta) < 0.00001)
					rewardHistory = -1;				
				newReward = true;	
				printf("new avgGoaDelta: rewardHistory = %f\n", rewardHistory);
			}

			// if it is the first frame, set a value for lastGoalDistance
